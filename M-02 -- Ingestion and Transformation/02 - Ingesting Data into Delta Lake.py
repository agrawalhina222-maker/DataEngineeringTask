...
# MAGIC %fs ls 'dbfs:/Volumes/dbacademy/ingesting_data/myfiles/'
# MAGIC
...
# MAGIC %sql
# MAGIC     
# MAGIC select * from csv.`dbfs:/Volumes/dbacademy/ingesting_data/myfiles/`
...
AS
SELECT ID, FirstName, Country, Role from csv.`dbfs:/Volumes/dbacademy/ingesting_data/myfiles/`
...
dbutils.fs.rm(f'/Volumes/{catalog_name}/{schema_name}/{volume_name}/employees2.csv', True)
...
df = spark.read.csv(f'/Volumes/{catalog_name}/{schema_name}/{volume_name}/employees2.csv', header=True, inferSchema=True)
from pyspark.sql.functions import when,lit,concat

updated_df = df.withColumn(
    'Country',
    when(df['Role'] == 'Developer', 'India').otherwise(df['Country'])
)
filtered_df = updated_df.filter(updated_df['Role'] == 'Developer').withColumn(
    'Firstname',concat(updated_df['Firstname'],lit('_')))
display(filtered_df)
...
## Create the new employees2.csv file in your volume
dbutils.fs.rm(f'/Volumes/{catalog_name}/{schema_name}/{volume_name}/employees3.csv', True)

# COMMAND ----------

#filtered_df.write.mode('overwrite').csv(f'/Volumes/{catalog_name}/{schema_name}/{volume_name}/employees4.csv', header=True)
# the above code creates files in partitions,, the way pyspark works

# Convert to pandas and write as a single CSV file
filtered_df.toPandas().to_csv(f'/Volumes/{catalog_name}/{schema_name}/{volume_name}/employees5.csv', index=False)

# COMMAND ----------


filtered_df.toPandas().to_csv(f'/Volumes/{catalog_name}/{schema_name}/{volume_name}/employees2.csv', index=False, header=True,mode='w')

# COMMAND ----------

# DBTITLE 1,SQL version of transformation
spark.sql(f"""
SELECT 
  ID,
  CONCAT(Firstname, '_') AS Firstname,
  CASE 
    WHEN Role = 'Developer' THEN 'India'
    ELSE Country 
  END AS Country,
  Role
FROM read_files(
