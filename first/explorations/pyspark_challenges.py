...
# ── Dataset 2: Traders (reference/dimension table) ────────────────────────────
traders_data = [
...
print("c) Total rows:", trades_df.count())

print("d) Distinct symbols:")
trades_df.select("symbol").distinct().show()

print("e) Summary stats:")
...
# ANSWER:
result_1_2 = trades_df.withColumn(
    "notional_m", F.round(F.col("notional") / 1_000_000, 4)
).withColumn(
    "is_large", F.col("notional") > 20000
).withColumn(
    "trade_label", F.concat_ws("-", F.col("side"), F.col("symbol"))
)
result_1_2.select("trade_id", "notional_m", "is_large", "trade_label").show()
...
# ANSWER:
print("a) BUY trades for AAPL or GOOG:")
trades_df.filter(
    (F.col("side") == "BUY") &
    (F.col("symbol").isin(["AAPL", "GOOG"]))
).show()

print("b) Notional between 10k and 50k:")
trades_df.filter(F.col("notional").between(10000, 50000)).show()

print("c) Price not null AND qty > 50:")
trades_df.filter(
...
# ANSWER:
result_2_2 = trades_df.withColumn(
    "risk_tier",
    F.when(F.col("notional").isNull(), "UNKNOWN")
     .when(F.col("notional") >= 100_000, "HIGH")
     .when(F.col("notional") >= 30_000,  "MEDIUM")
     .otherwise("LOW")
)
result_2_2.groupBy("risk_tier").count().orderBy("risk_tier").show()
...
# ANSWER:
result_3_1 = trades_df.groupBy("symbol").agg(
...
# ANSWER:
trader_region = trades_df.groupBy("region", "trader").agg(
    F.sum("notional").alias("total_notional")
)

w = Window.partitionBy("region").orderBy(F.desc("total_notional"))

result_3_2 = trader_region \
...
# ANSWER — Option A (Window, elegant):
w_total = Window.rowsBetween(Window.unboundedPreceding, Window.unboundedFollowing)

result_3_3 = trades_df.groupBy("symbol").agg(
    F.sum("notional").alias("total_notional")
).withColumn(
    "grand_total", F.sum("total_notional").over(w_total)
).withColumn(
    "pct_of_total", F.round(F.col("total_notional") / F.col("grand_total") * 100, 2)
).drop("grand_total")

result_3_3.orderBy(F.desc("pct_of_total")).show()
...
# ANSWER:
w_sym = Window.partitionBy("symbol").orderBy("price_date")
w_run = Window.partitionBy("symbol").orderBy("price_date") \
              .rowsBetween(Window.unboundedPreceding, Window.currentRow)
w_roll3 = Window.partitionBy("symbol").orderBy("price_date") \
                .rowsBetween(-2, Window.currentRow)  # -2 = 2 rows back + current = 3

result_4_1 = prices_df \
    .withColumn("prev_close",   F.lag("close_price", 1).over(w_sym)) \
    .withColumn("daily_return", F.round(
        (F.col("close_price") - F.col("prev_close")) / F.col("prev_close") * 100, 2
    )) \
    .withColumn("rolling_3d_avg", F.round(F.avg("close_price").over(w_roll3), 2)) \
    .withColumn("cumulative_max", F.max("close_price").over(w_run))

result_4_1.filter(F.col("symbol") == "AAPL").show()
...
# ANSWER:
w_dedup = Window.partitionBy("symbol", "trade_date", "side", "trader") \
                .orderBy("trade_id")
...
# ANSWER:
w_trader = Window.partitionBy("trader").orderBy("trade_date")

result_4_3 = trades_df.select("trader", "trade_date").distinct() \
    .withColumn("prev_date", F.lag("trade_date", 1).over(w_trader)) \
    .withColumn("day_diff",  F.datediff(F.col("trade_date"), F.col("prev_date"))) \
    .withColumn("is_new_streak",
        F.when(F.col("day_diff").isNull() | (F.col("day_diff") > 1), 1).otherwise(0)
    ) \
    .withColumn("streak_id",
        F.sum("is_new_streak").over(
            Window.partitionBy("trader").orderBy("trade_date")
                  .rowsBetween(Window.unboundedPreceding, Window.currentRow)
        )
    ) \
    .select("trader", "trade_date", "streak_id")

result_4_3.orderBy("trader", "trade_date").show(20)
...
#
# This CANNOT be done with a simple window — requires a different approach.
# Hint: Use a Python UDF or convert to Pandas UDF for stateful logic.
# -----------------------------------------------------------------------------
print("\n--- Challenge 4.4: Running Total with Reset (Stateful) ---")
# HINT: pandas_udf with GROUPED_MAP — process each trader's data as a pandas df
...
<workspaces-note>DO NOT REMOVE|fc93d63baee614aa9bb7d590bc473740dfdc9331ee324ac65cd04230f40eee54</workspaces-note>
